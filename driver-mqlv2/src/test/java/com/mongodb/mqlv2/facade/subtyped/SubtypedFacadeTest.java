...
import static com.mongodb.mqlv2.facade.subtyped.Subtyped.arr;
import static com.mongodb.mqlv2.facade.subtyped.Subtyped.asc;
import static com.mongodb.mqlv2.facade.subtyped.Subtyped.assign;
import static com.mongodb.mqlv2.facade.subtyped.Subtyped.bag;
import static com.mongodb.mqlv2.facade.subtyped.Subtyped.current;
import static com.mongodb.mqlv2.facade.subtyped.Subtyped.dateLit;
import static com.mongodb.mqlv2.facade.subtyped.Subtyped.dateField;
import static com.mongodb.mqlv2.facade.subtyped.Subtyped.desc;
import static com.mongodb.mqlv2.facade.subtyped.Subtyped.doc;
import static com.mongodb.mqlv2.facade.subtyped.Subtyped.docField;
...
import static com.mongodb.mqlv2.facade.subtyped.Subtyped.intVar;
import static com.mongodb.mqlv2.facade.subtyped.Subtyped.isNullish;
import static com.mongodb.mqlv2.facade.subtyped.Subtyped.letIn;
import static com.mongodb.mqlv2.facade.subtyped.Subtyped.litMissing;
import static com.mongodb.mqlv2.facade.subtyped.Subtyped.litNull;
import static com.mongodb.mqlv2.facade.subtyped.Subtyped.strLit;
import static com.mongodb.mqlv2.facade.subtyped.Subtyped.sum;
import static org.junit.jupiter.api.Assertions.assertEquals;

class SubtypedFacadeTest {
    ...
    private List<BsonDocument> run(final PipelineBuilderS pipeline) {
    ...
    private static void assertSameAst(final Stage facadeAst, final Stage bareAst) {
        assertEquals(bareAst, facadeAst, "subtyped facade AST diverges from bare AST");
    }
    ...
    void formatWithDocAndMul() {
        ...
        assertSameAst(facade.stage(), bare);
        assertEquals(bset(bd("{\"doubled\": 2}"), bd("{\"doubled\": 4}")), asSet(run(facade)));
    }
    ...
    void sortDescThenAsc() {
        ...
        PipelineBuilderS facade =
                from(bag(
                        doc(entry("a", intLit(3L)), entry("b", intLit(2L))),
                        doc(entry("a", intLit(1L)), entry("b", intLit(7L))),
                        doc(entry("a", intLit(5L)), entry("b", intLit(2L))),
                        doc(entry("a", intLit(5L)), entry("b", intLit(1L)))))
    ...
    void projectBranching() {
        ...
        PipelineBuilderS facade =
                from(bag(doc(
                        entry("a", doc(entry("x", intLit(1L)), entry("y", intLit(2L)), entry("z", intLit(3L)))),
                        entry("b", intLit(9L)))))
                        .project("a.x", "a.z", "b");
    ...
    void setWithArithmetic() {
        ...
        PipelineBuilderS facade =
                from(bag(doc(entry("a", intLit(1L))), doc(entry("a", intLit(2L)))))
                        .set(assign("z.add1", intField("a").add(intLit(1L))));
    ...
    void unset() {
        ...
        PipelineBuilderS facade =
                from(bag(doc(entry("a", intLit(1L)), entry("b", intLit(2L)))))
                        .unset("b");
    ...
    void distinctAndCount() {
        PipelineBuilderS facadeDistinct =
                from(bag(intLit(1L), intLit(1L), intLit(2L), intLit(3L)))
                        .distinct();
        ...
        PipelineBuilderS facadeCount =
                from(bag(intLit(1L), intLit(2L), intLit(3L), intLit(4L), intLit(5L)))
                        .count();
    ...
    void unwindSimple() {
        // from <<[1,2,3]>> | unwind $
        PipelineBuilderS facade =
                from(bag(arr(intLit(1L), intLit(2L), intLit(3L))))
                        .unwind(current());
    ...
    void anyExpressionAndArrow() {
        ...
        PipelineBuilderS facade =
                from(bag(
                        doc(entry("a", arr(doc(entry("b", intLit(1L))), doc(entry("b", intLit(2L)))))),
                        doc(entry("a", arr(doc(entry("b", intLit(3L))))))))
                        .match(field("a").arrArrow("b").any(intCurrent().eq(intLit(2L))));
        Stage bare = new Stage.MatchStage(
                new Stage.FromStageSimple(bBag(
    ...
    void groupWithSumArrow() {
        ...
        assertSameAst(facade.stage(), bare);
        assertEquals(bset(bd("{\"k\": 1, \"s\": 5}"), bd("{\"k\": 2, \"s\": 4}")), asSet(run(facade)));
    }

    @Test
    void letExpr() {
        // from let $x = 2 in $x + 3
        PipelineBuilderS facade = from(letIn(intVar("x").add(intLit(3L)), entry("x", intLit(2L))));
    ...
    void topLevelAggViaFunctionCall() {
        // from sum(<<1,2,3,4>>)
        PipelineBuilderS facade = from(sum(bag(intLit(1L), intLit(2L), intLit(3L), intLit(4L)).asInt()));
        Stage bare = new Stage.FromStageSimple(
    ...
    void dateExtractors() {
        ...
        PipelineBuilderS facade =
                from(bag(doc(entry("dt", dateLit(dtMillis)))))
                        .format(doc(
                                entry("y",  dateField("dt").year()),
                                entry("m",  dateField("dt").month()),
                                entry("dm", dateField("dt").dayOfMonth()),
                                entry("dy", dateField("dt").dayOfYear()),
                                entry("dw", dateField("dt").dayOfWeek()),
                                entry("h",  dateField("dt").hour()),
                                entry("mn", dateField("dt").minute()),
                                entry("s",  dateField("dt").second()),
                                entry("ms", dateField("dt").millisecond())));
    ...
    void dateComparisons() {
        ...
        // dateLit(jan1) < dateLit(dec31)  => true
        PipelineBuilderS facadeLt = from(dateLit(jan1).lt(dateLit(dec31)));
        ...
        // dateLit(mid) == dateLit(mid)  => true
        PipelineBuilderS facadeEqTrue = from(dateLit(mid).eq(dateLit(mid)));
        ...
        // dateLit(jan1) == dateLit(dec31)  => false
        PipelineBuilderS facadeEqFalse = from(dateLit(jan1).eq(dateLit(dec31)));
        ...
        // dateLit(mid) is dateLit(mid)  => true
        PipelineBuilderS facadeIs = from(dateLit(mid).is(dateLit(mid)));
