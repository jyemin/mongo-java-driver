...
final class MongoClientDelegate {
    ...
    private class DelegateOperationExecutor implements OperationExecutor {
        ...
        public <T> T execute(final ReadOperation<T> operation, final ReadPreference readPreference, final ReadConcern readConcern,
            ...
            ClientSession clientSession = getClientSession(clientSessionFromOperation);
            ReadBinding binding = getReadBinding(readPreference, readConcern, clientSession, clientSessionFromOperation == null);

            try {
                if (clientSession.hasActiveTransaction() && !binding.getReadPreference().equals(primary())) {
        ...
        public <T> T execute(final WriteOperation<T> operation, final ReadConcern readConcern,
            ...
            ClientSession clientSession = getClientSession(clientSessionFromOperation);
            WriteBinding binding = getWriteBinding(readConcern, clientSession, clientSessionFromOperation == null);
        ...
        private void clearTransactionContextOnTransientTransactionError(final ClientSession session, final MongoException e) {
            if (e.hasErrorLabel(TRANSIENT_TRANSACTION_ERROR_LABEL)) {
                session.clearTransactionContext();
            }
        }

        private ReadPreference getReadPreferenceForBinding(final ReadPreference readPreference, final ClientSession session) {
            if (session.hasActiveTransaction()) {
        ...
        ClientSession getClientSession(@Nullable final ClientSession clientSessionFromOperation) {
            if (clientSessionFromOperation != null) {
                ...
                return createClientSession(ClientSessionOptions.builder().causallyConsistent(false).build(), ReadConcern.DEFAULT,
