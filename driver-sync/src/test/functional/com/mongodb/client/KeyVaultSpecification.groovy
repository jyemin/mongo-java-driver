...
import static com.mongodb.client.Fixture.mongoClientSettings

class KeyVaultSpecification extends FunctionalSpecification {

    private MongoNamespace keyVaultNamespace = new MongoNamespace('test.datakeys')
    private MongoCollection dataKeyCollection = getMongoClient()
            .getDatabase(keyVaultNamespace.databaseName).getCollection(keyVaultNamespace.collectionName, BsonDocument)
    private Map<String, Map<String, ? extends Object>> localProviderProperties =
            ['local':
                     ['key': Base64.decoder.decode('HGq6Qg6q28P6EdlDF2+lELxTEqa107jqF4zhXN64ElhUIqv1LwvPy5ldQqTnk1XAGX/uSmkY1R3z0mtDHXxg9A==')]]
    private Map<String, Map<String, Object>> awsProviderProperties =
    ...
    private BsonDocument awsMasterKey = new BsonDocument('region', new BsonString('us-east-1'))
            .append('key', new BsonString('arn:aws:kms:us-east-1:579766882180:key/89fcc2c4-08b0-4bd9-9f25-e30687b580d0'))
    private byte[] inititializationVector = Base64.decoder.decode('aWlpaWlpaWlpaWlpaWlpaQ==')
    private BsonDocument localDataKeyDocument = BsonDocument.parse(
            '{' +
                    '  "_id": {' +
                    '    "$binary": {' +
                    ...
                    '  }' +
                    '}')
    private BsonDocument awsDataKeyDocument = BsonDocument.parse(
            '{' +
                    '  "_id": {' +
                    '    "$binary": {' +
